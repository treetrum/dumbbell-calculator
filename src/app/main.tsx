...
const createIdentity = (plates: Plate[]) => {
  return JSON.stringify(plates.sort((a, b) => b.weight - a.weight).map((p) => p.weight));
};
    ...
    <div style={{ padding: 16, maxWidth: 800, margin: "0 auto" }}>
      ...
      <div style={plateGridStyles}>
        {availablePlates.map((plate) => (
          <button
            disabled={usedPlates.map((p) => p.id).includes(plate.id)}
        ...
        {usedPlates
          ...
          .map((plate) => (
            <button
              ...
              key={plate.id}
              onClick={() => {
                setUsedPlates((old) => old.filter((p) => p.id !== plate.id));
              }}
            >
              {plate.weight}
            </button>
      ...
      <div style={plateGridStyles}>
        {uniquePlateCombinations
          ...
          .map((combination) => {
            ...
            const isActive =
              JSON.stringify(combination.sort((a, b) => b.weight - a.weight)) ===
              JSON.stringify(usedPlates.sort((a, b) => b.weight - a.weight));
            return (
              <button
                key={singleDumbbellWeight}
                style={{
                  ...plateButtonStyles,
                  ...(isActive ? activeButtonStyles : undefined),
                }}
                onClick={() => {
